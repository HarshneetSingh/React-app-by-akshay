...
const FilterBar = (props) => {
    ...
    return (
        ...
        <>
            <div className={`fixed w-2/5 h-full top-0 right-0 bg-white z-50 transition-all duration-[400ms] overflow-auto ${(props.filterBarState === false) ? "translate-x-[620px] opacity-0" : "translate-x-0 "}`}>
                {/* filter header */}
                                    ...
                                    <div className='w-full  grid grid-cols-2'>
                                        {
                                            filterName?.options.map((filter, index) => {
                                                ...
                                                return <FilterBarBtn key={index} index={index} filterName={filterName} filterArr={filterArr} setFilterArr={setFilterArr} filter={filter} />
                                            })
                                        }
                ...
                <div className=' pl-10 sticky bottom-0 font-bold bg-white text-sm w-full py-5 shadow-[0_-2px_4px_0_#e9e9eb] left-0  '>
                    <button className='h-[50px]  border border-lightColor px-7 mr-4   text-lightColor opacity-50' onClick={
                        () => {
                            setFilterArr({
                                CUISINES: [],
                    ...
                    <button onClick={() => {
                        setUrl({ filter: `` })
                        restroSorting(selectedSort.sort,props.setFilteredRestaurants,props.location,)
                        props.setFilteredRestaurants([])

                    }} className='h-[50px] px-10 text-white bg-headerHoverColor  border-headerHoverColor shadow-[0_1px_3px_0_rgba(0,0,0,.12)] hover:shadow-[0_2px_8px_#d4d5d9] border'>SHOW RESTAURANTS</button>
                </div>
            </div>
