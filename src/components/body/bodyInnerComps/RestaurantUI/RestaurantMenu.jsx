...
const RestaurantMenu = () => {
  ...
  restroDishes = (restroDishes?.card?.card?.hasOwnProperty('categories'))
          ...
          <div className='flex flex-nowrap overflow-x-scroll yo gap-x-3'>
            {
              offersOnRestaurant.map((offer) => {
                return (
                  <div className='flex border rounded-md border-restroCardBorder p-3   shadow-[0_1px_2px_[rgba(0,0,0,.04)]'>
                    {
                      (offer?.info.hasOwnProperty('offerTag'))
                        ?
                        <p className='vertical-lr rotate-180 border-l pl-1 mr-2 text-darkOrange font-semibold text-[0.7rem] border-inherit'>{offer?.info?.offerTag}</p>
                        : ""
                    }
