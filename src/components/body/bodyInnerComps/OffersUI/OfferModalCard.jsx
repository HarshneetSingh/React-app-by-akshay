...
const OfferModalCard = (props) => {
  ...
  return (
    <>
      <div className={` bg-white fixed w-1/4  p-7 top-2/4  left-2/4   overflow-auto transition-all  -translate-x-2/4 duration-500 z-50 ${(offerModalState === false) ? "translate-y-full opacity-0 duration-[0ms]" : " -translate-y-2/4"}`}>
        <div className='flex'>
          <div className='flex items-center justify-around border border-[#daceb7]  bg-[#fffae6]  '>
            <img src={`https://res.cloudinary.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_40,h_40/${offerModalData?.logo}`} className="w-5 h-5 mx-2 " alt="" />
        ...
        <div className='text-xs my-2'>{offerModalData?.tnc?.title}</div>

        <div>
          {
            offerModalData?.tnc?.bulletTexts.map((text, index) => {
              return <div key={index} className='flex  items-center'>
                <span className='font-black  text-xl'> &#183;</span> <span className='ml-2  text-xs text-sortByBtnColor'> {text}</span>
              </div>
            })
          }
        </div>
